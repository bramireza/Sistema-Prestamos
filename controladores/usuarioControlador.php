	...
	class usuarioControlador extends usuarioModelo{
		...
		public function actualizar_usuario_controlador(){
			...
			if (isset($_POST['usuario_privilegio_up'])) {
				...
				$estado=$campos['usuario_privilegio'];
			}
			...
			$admin_clave=mainModel::encryption($_POST['clave_admin']);
			...
			if($email!=$campos['usuario_email'] &&  $email!= ""){
				if(filter_var($email,FILTER_VALIDATE_EMAIL)){
					$check_email=mainModel::ejecutar_consulta_simple("SELECT usuario_email FROM usuario WHERE usuario_email='$mail'");
					if($check_email->rowCount()>0){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
			...
			if($_POST['usuario_clave_nueva_1'] != "" || $_POST['usuario_clave_nueva_2'] != ""){
				if($_POST['usuario_clave_nueva_1'] != $_POST['usuario_clave_nueva_2']){
					$alerta=[
						"Alerta"=>"simple",
						"Titulo"=>"Ocurrió un error inesperado",
						"Texto"=>"Las nueas claves no coinciden",
						"Tipo"=>"error"
					];
					echo json_encode($alerta);
					exit();
					...
					$_POST["usuario_clave_nueva_2"]) ){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"Las nueas claves no coinciden con el formato solicitado",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();
			...
			if ($tipo_cuenta=="Propia") {
				...
				WHERE usuario_usuario='$admin_usuario' AND usuario_clave='$admin_clave' AND
				 usuario_id='$id'");
			} else {
				session_start(["name"=>"SPM"]);
				if($_SESSION['privilegio_spm']!=1){
